...
/**
 * The application data is transmitted over the communication medium via one or
 * a concatenation of consecutive data blocks. A data block shall consist of: -
 * a one octet field indicating the Asterix category - a two octet field
 * indicating the length of the data block, including the CAT and LEN fields
 */
public class AsterixDataBlock extends DebugMessageSource implements Decodable {
    ...
    /**
     * Create an Asterix data block for the specified category with a custom
     * factory implementation for SP,RE fields.
     *
     * @param category
...
//        Assert.notNull(reservedFieldFactory);
    ...
    /**
     * Encodes the current Asterix data block into a byte array, that can be
     * then sent over the network or written to a file.
     *
     * @param dest The destination array
     * @param offset The start offset in array dest at which writing should
     * begin
